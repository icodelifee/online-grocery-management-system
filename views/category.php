...
$products;
if (isset($_GET["id"])) {
    $select = "SELECT cat_name FROM tbl_category WHERE cat_id=" . $_GET["id"];
    if ($result = $connect->query($select)) {
        $catName = $result->fetch_array();
    } else {
        echo $connect->error;
    }
    $fetchSubcat = "select * from tbl_subcategory where cat_id=" . $_GET["id"];
    if ($subcatRes = $connect->query($fetchSubcat)) {
        $subCats = $subcatRes->fetch_all();
    } else {
        echo $connect->error;
    }
    $fetchProducts = "select * from tbl_item where cat_id=" . $_GET["id"];
                ...
                <div class="col">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="category.php?id=<?php echo $_GET["id"] ?>"><?php echo $catName[0] ?></a></li>
                        </ol>
                    </nav>
                </div>
            </div>
